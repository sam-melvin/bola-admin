          ...
          <div class="col-12">
              <div class="card">
                    <div class="card-header">
                      <h3 class="card-title">Digital Tally Sheets Pending</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                    <form action="sendDigitalTally.php" method="post">
                      <table id="example1" class="table table-bordered table-striped">
                        ...
                        <?php
                                        $status = "pending";
                                        $managerCode = $_SESSION["ucode"];
                                        $sql = "SELECT * FROM `digital_tally` WHERE `status`='$status' AND `manager_code`='$managerCode'";
    ...
    <?php
      ...
      ?>
                ...
                if ($(this).is(':checked')) {
                  ...
                  // total_earnings -= getTotalEarnings(y);
                  let elsedataString = ($(this).attr("data-id"));
                  var conArray = JSON.parse("[" + elsedataString + "]");
                  subtotal -= parseFloat(conArray[1]);
                  // subtotal -= parseFloat(y);
                  // total_payout = getTotalPayout(subtotal,total_earnings);
                  // subtotalForm.value = subtotal.toFixed(2);
                  // total_earningsForm.value = total_earnings.toFixed(2);
                  // total_payoutForm.value = total_payout.toFixed(2);
                  // payout_array = payout_array.filter(function( obj ) {
                  //     return obj.id !== conArray[0];
                  // });
                  
                  
                  console.log("conArray[0]: " + conArray[0]);
                  payout_array = payout_array.filter(function(item) {
                      return item.id !== conArray[0]
                  })

                  // payout_array.splice(payout_array.findIndex(item => item.id === conArray[0]));
