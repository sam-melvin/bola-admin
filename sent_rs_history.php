...
if (USER_TYPE_MANAGER === $loggedUser->type) {
    ...
    $commissionField = 'com_percent_man as commission_rate';
    $payout = 'sum(grand_total_man) as payout';
} else {
    $columnFilterName = 'manager_code';
    $commissionField = 'com_percent_sup as commission_rate';
    $payout = 'sum(grand_total_sup) as payout';
}

$query = DigitalTally::where($columnFilterName, $loggedUser->user_id_code)
    ->select('date_submit','draw_number','status')
    ->selectRaw('sum(sub_total) as total')
    ->selectRaw($commissionField)
    ->selectRaw($payout)
    ->groupBy('draw_number')
        ...
        <div class="content-wrapper">
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Report Summary History</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="/">Home</a></li>
                                <li class="breadcrumb-item"><a href="sent_digital.php">Sent Tally</a></li>
                                <li class="breadcrumb-item active">Report Summary History</li>
                            </ol>
                        </div>
                    </div>
                </div>
                                        ...
                                        <tbody>
                                            <?php
                                                foreach ($lists as $the):
                                                    $earnings = $the->total - $the->payout;
                                            ?>
                                                <tr>
                                                    <td><?= $the->date_submit ?></td>
                                                    <td><?= $the->draw_number ?></td>
                                                    <td>&#8369;<?= number_format($the->total,2) ?></td>
                                                    <td>&#8369;<?= number_format($the->payout,2) ?></td>
